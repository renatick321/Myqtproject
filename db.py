...
def user_create(login, password, name, age, location, description, img_path):
	...
	message_create('Вы можете писать сами себе!', login, login)
	conn.commit()
...
def user_update(login, name, password, age, location, description, img_path):
	cursor.execute("UPDATE Users SET password = '{}' WHERE login = '{}'".format(password, login))
	cursor.execute("UPDATE Users SET name = '{}' WHERE login = '{}'".format(name, login))
